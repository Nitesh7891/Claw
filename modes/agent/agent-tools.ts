...
export function createAgentTools(executor:ToolExecutor){
    return {
        ...
        read_file:tool({
           description:
           "Read a text file from the workspace. use a path relative to the roject root",
        ...
        delete_file:({
            description:
            "Stage deletion of a new file (pending approval)",
            inputSchema:z.object({
                path:z.string(),
            }),
        ...
        create_folder:tool({
           description:
           "Stage creation of a directory tree (pending approval).Use mkdir -p on apply.",
           inputSchema:z.object({
        ...
        search_files:tool({
            description:
            'Find files matching a glob pattern (e.g. "*.ts"args, "**/*.md"). Optional content substring filter.',
            inputSchema:z.object({
