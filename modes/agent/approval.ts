...
export async function runApprovalFlow(tracker: ActionTracker): Promise<boolean> {
  const pendingApprovals = tracker.getPendingMutations();
  ...
  const choice = await select({
    message: "Do you want to apply staged changes ?",
    options: [
      { value: "all", label: "Approve and apply all" },
      { value: "select", label: "Review one by one and then select changes" },
